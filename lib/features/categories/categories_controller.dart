...
import '../../dtos/api_dtos/categories_screen/selectusercategory/selectusercategory_request.dart';
...
class CategoriesController extends GetxController with SnackbarMixin {
  ...
  // final _preSelectedcategoryFetch = Rx<FetchUserCategoriesModel?>(null);
  // FetchUserCategoriesModel? get preSelectedcategoryFetch => _preSelectedcategoryFetch.value;
  ...
  userCategoryFetchedSyncing() async {
    _numberOfCategorySelected.value = 0;

    for (int i = 0; i < categoryFetchConverted.length; i++) {
      bool idExists = preUsercategoryContentFetch.any((element) =>
          element.categoryId == categoryFetchConverted[i].categoryId);
  ...
  fetchingUserCategory() async {
    _isLoadingUserCategoryFinished(false);

    _preUsercategoryFetch.value = null;
    _preUsercategoryContentFetch.value = [];
    ...
    try {
      final request =
          FetchUserCategoryRequest(apikey: apikey!, userId: userId!);
      final response =
          await ApiRepository.to.fetchUserCategory(request: request);

      if (response.status == 200) {
        _preUsercategoryFetch.value = response.data;
        if (preUsercategoryFetch != null) {
          _preUsercategoryContentFetch.value = preUsercategoryFetch!.content;
          await userCategoryFetchedSyncing();
        }
